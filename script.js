...
- Preserve an author's effective, consistent approach; do not rewrite just to match your personal preferences.
- Prefer revision over deletion. Do not remove content without a solid reason. Do not shorten text merely to reduce length.
...
- Remove weak filler words unless they add meaning (for example, "actually", "really", "kind of", "to be honest").
...
- Define acronyms on first use (for example, CLI, PR, SDK).
- Use standard Markdown for reader aids and headings. Keep formatting light.
...
## LaTeX
- Use LaTeX for mathematical symbols and equations in math contexts. Use inline LaTeX (\`$...$\`) only for math; do not wrap ordinary numerals, units, dimensions, or percentages.

## Programming Terminology and Preferred Spelling
- Respect programming idioms even when they sound unusual. Do not add articles before "code". Use the house word list for spelling, casing, and hyphenation, for example, backend, DataFrame, log in, runtime, web page, vs.
...
- Return ONLY the fully edited and revised text that preserves the author's voice, improves readability, and strengthens structure.
...
Now proofread and edit the following course or lesson text. Paste the text between the markers.`;
...
const SECONDARY_FALLBACK_MODEL = 'gemma-2-27b-it';
...
async function fetchWithFallback(apiKey, requestBody, fallbackLevel = 0) {
    // Select model based on fallback level: 0 = primary, 1 = flash-lite, 2 = gemma
    let model;
    if (fallbackLevel === 0) {
        model = PRIMARY_MODEL;
    ...
    if (!response.ok) {
        ...
        if (modelUnavailable || quotaExceeded) {
            const reason = quotaExceeded ? 'quota exceeded' : 'unavailable';

            if (fallbackLevel === 0) {
                // Try first fallback (Flash Lite)
                console.log(`Primary model (${PRIMARY_MODEL}) ${reason}, trying fallback model (${FALLBACK_MODEL})`);
                return fetchWithFallback(apiKey, requestBody, 1);
            } else if (fallbackLevel === 1) {
                // Try second fallback (Gemma)
                console.log(`First fallback model (${FALLBACK_MODEL}) ${reason}, trying secondary fallback (${SECONDARY_FALLBACK_MODEL})`);
                return fetchWithFallback(apiKey, requestBody, 2);
            }
        }
...
async function proofreadText() {
    ...
    try {
        ...
        if (model === FALLBACK_MODEL) {
            setStatus('Using Gemini 2.5 Flash Lite (primary model quota exceeded)', 'normal');
        } else if (model === SECONDARY_FALLBACK_MODEL) {
            setStatus('Using Gemma 2 27B (Flash models quota exceeded)', 'normal');
        }
...
async function trimText() {
    ...
    const targetLength = Math.floor(limit * 0.80);

    // Set loading state
    trimBtn.disabled = true;
    trimBtnText.innerHTML = '<span class="spinner"></span> Trimming...';
    setStatus('Trimming text...', 'normal');

    const startTime = Date.now();

    try {
        const trimPrompt = `You are an expert editor. Shorten the following text to approximately ${targetLength} characters (80% of the ${limit} character limit) while preserving the core message and maintaining quality.

IMPORTANT RULES:
- Target length: ${targetLength} characters (current: ${currentText.length} characters)
...
${currentText}`;
        ...
        // Notify user if fallback model is being used
        if (model === FALLBACK_MODEL) {
            setStatus('Trimming with Gemini 2.5 Flash Lite (primary model quota exceeded)', 'normal');
        } else if (model === SECONDARY_FALLBACK_MODEL) {
            setStatus('Trimming with Gemma 2 27B (Flash models quota exceeded)', 'normal');
        }
