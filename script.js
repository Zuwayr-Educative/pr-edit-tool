// System prompt loaded from systemprompt.md
let SYSTEM_PROMPT = '';

// Load system prompt on startup
async function loadSystemPrompt() {
    try {
        const response = await fetch('systemprompt.md');
        if (!response.ok) {
        ...
        // Fallback to default if file fails to load
        SYSTEM_PROMPT = 'You are an expert editor. Proofread and improve the following text.';
    }
}
